...
이 시스템은 SQL 파일 또는 interactive 입력을 받아 `INSERT`와 `SELECT`를 CSV 파일에 대해 실행하는 최소 구현 SQL Processor입니다.
...
- 파일 내용을 읽거나 interactive 입력을 받는다.
- 파싱과 실행을 시작한다.
- 결과를 화면에 출력한다.
...
현재 구현에서 `SELECT` 결과는 CSV 줄 목록이 아니라 표 형식 문자열로 출력됩니다.
